...
export const getDefaultValues = (profile: Partial<UserProfile>): RawPayload => {
  const DEFAULT_PHONE = '+7 ('

  if (!profile) {
    return {
      [FIELD_NAME_FIO]: '',
      [FIELD_NAME_PHONE]: DEFAULT_PHONE,
      [FIELD_NAME_PASSPORT]: '',
      [FIELD_NAME_ADDRESS]: ''
    }
  }
  ...
  return {
    [FIELD_NAME_FIO]: fio,
    [FIELD_NAME_PHONE]: profile[FIELD_NAME_PHONE] ?? DEFAULT_PHONE,
    [FIELD_NAME_PASSPORT]: profile[FIELD_NAME_PASSPORT] ?? '',
    [FIELD_NAME_ADDRESS]: profile[FIELD_NAME_ADDRESS] ?? ''
  }
}
